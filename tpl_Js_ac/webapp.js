...
$(document).on("pageInit", function (e, pageId, $page) {
   ...
   // console.log(this_page_historyID);
    ...
    if(pageId == "page-index") {
        if(notetitle){$('.notetitle').html(notetitle);$('.notecontent').html(notecontent);}//设置公告
        if(app_var){$('.app_var').html(app_var);}//版本信息
        //一句话小贴士
        if(webapp_tips){
    ...
    if(pageId == "page-search") {
        if ( localStorage.getItem("searchwd")){
            localStorage_update_searchwd(); //历史记录 关键词
        }
...
function open_Show_vod_guset(){
    ...
    if(webapp_tips){
        var temp_html='';
        var temp_x=0;
        $.each(webapp_tips, function (i) {
            temp_x=i+1;
            temp_html+=temp_x+" : "+this+"<br>";
        });
        $('.play_tips').html(temp_html);
    }
    $.popup('.Show_Vod_to_guest_class'); //打开层
}
//格式化一条视频信息到弹窗
// 写到单独函数里面 方便复用
        ...
        $.ajax({
            ...
            success: function (r) {
                if (r.code == 1) {
                    ...
                    $(".updatethisvoding").html("已同步最新数据： 今天 "+temp_str);
...
function localStorage_update_searchwd() {
    ...
    if (temp_seadrchwd) {
        ...
        $("#YourKeywords").html(temphtml + "[<a href='javascript:localStorage_del_searchwd();'>删除</a>]<div class='clear'></div>");//填充内容
    }
}
//localStorage 删除搜索记录
...
//热搜关键词
function get_hotKeyWords() {
    if(hotKeyWords){
        var arr_hotKeyWords = hotKeyWords.split("|")
        var temp_html = "";
...
function HengShuPing() {
    if (window.orientation == 180 || window.orientation == 0) {
        //竖屏
        $("#Show_Vod_to_guest #No_hengping1").after(add_tr);//恢复之前删除 
...
/* 首页通知公告查看更多内容 */
function Div_show_hide(div_class,button_class,button_str){
    if($("."+div_class).is(":hidden")){
        $("."+div_class).show();
        $("."+button_class).html(" &lt;&lt;&lt;&lt;"); //<<<<
    }else{
        $("."+div_class).hide();
        $("."+button_class).html(button_str);
    }
}